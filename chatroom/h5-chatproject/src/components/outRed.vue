<template>
  <div class="regist-content">
    ...
    <p style="left: 38%" class="top-font">收到的红包</p>
    <img @click="showItem" width="22" height="4" style="position: absolute; top: 18px; right: 20px;z-index: 102"
          ...
          <li v-for="(item, index) in outList" :key="index">
            ...
            <div class="list-right-can">
              <div style="width: calc(100vw - 27px);" class="list-right-top">
                <p>{{'红包'}}</p>
                <p style="color: #EE0024">{{item.amount}}</p>
              </div>
              <span style="margin-top: -12px">{{item.time}}</span>
            </div>
          </li>
        </ul>
      </div>
    </div>
    <div v-if="showBtn" class="list-menu">
      <ul>
        <li @click="toGetRed">
          <p>收到的红包</p>
        </li>
        ...
        <li @click="showItem" style="border-top: 4px solid #dbdbdb">
          <p>取消</p>
        </li>
      </ul>
    </div>
