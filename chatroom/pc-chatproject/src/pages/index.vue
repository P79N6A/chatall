<template>
      ...
      大家啊上看到
            ...
            <div class="aite "  @click="goAnchor()"  v-if="isAiTeMe && aiTeMe >0" >
				...
				<el-scrollbar id="messageScroll" :native="false" :wrap-style="sHeigth">
					<div id="msgcon" class="message-context">
						<div @click="getMessageHistory(false)" class="view-more-records"><a class="more-message" page="1">查看更多消息</a></div>
						...
						<template v-for="(msg,index) in messageList" >
							<div class="message-wrap" v-if="msg.type =='red' &&  msg.redpackettype == '0'   ">
                ...
                <!-- 系统消息 -->
								<div :id="[msg.isAiTeMeMsg?('anchor-'+msg.time):'' ]" v-if="msg.mtype === 'user'&&!switcher" class="chat-item notmine " :class="[msg.flag == 2 ?'teacher':'',msg.mtype=='system'?'system':'',msg.mtype=='customer'?'customer':'',msg.mtype=='redpacket'?'redpacket':'']">
									<!-- 图片消息 -->

                  <div class="user-img" @dblclick="noticeTo(msg.nickname,msg.mtype)" title="双击@他">
										<div class="uimg" >
                     <img v-if="msg.flag == 2" :src="imgUrl+'images/avatar/kefu.png'">
                      <img v-else  :src="imgUrl+'images/avatar/'+msg.avatar">
                  ...
                  <!-- 红包消息 撤回消息 -->
                     ...
                     <img v-if=" msg.flag == 2" :src="imgUrl+'images/grade/kefu_logo.png'" class="grade_type" style="width:60px ;border-radius: 12px;">
                      ...
                      <img v-if="msg.grade&&(msg.flag == 1 || msg.flag == 3 )" :src="imgUrl+'images/grade/'+msg.grade+'.png'" class="grade_type" style="width:60px">
                      <img v-if="msg.user_degree&&(msg.flag == 1 || msg.flag == 3 )" :src="imgUrl+'images/grade/'+msg.user_degree+'.png'" class="grade_type" style="width:60px">
                      <!-- <img v-if="msg.flag" :src="imgUrl+'images/avatar/kefu.png'" class="grade_type" style="width:60px ;border-radius: 12px;">
                      <img v-if="!msg.flag"  :src="imgUrl+'images/avatar/'+msg.avatar" class="grade_type" style="width:60px ;border-radius: 12px;"> -->
                      ...
                      class="chat-info-2" v-if="msg.mtype === 'user' ">
                        <div
													...
													z-index: 1001;
													cursor: pointer;
													position: absolute; width: auto; height: 50px; display: flex; border-radius: 5px;"
                          :style="'left:'+left+'px'"
													class="contextmenu" >
													<template>
														<div

														style="text-align: center;width: auto; display: flex; margin-top: 5px;"  class="li">
															<div class="icon" style="display:inline-block;width: 50px; margin: 5px 10px 0 10px; text-align: center " @click="deleteMessage" v-if="myFlag =='2' && (msg.flag !==2 || isMe) " >
                                <p><Icon
                              ...
                              <div class="icon" style="display:inline-block;width: 50px; margin: 5px 10px 0 10px;  text-align: center" @click="outMessage(msg.msgid)" v-if="myFlag =='2' || myFlag =='3'">
                                <p><Icon
																style="color: #fff; font-size: 20px;"
                              ...
                              <div class="icon" style="display:inline-block;width: 50px;margin: 5px 10px 0 10px;  text-align: center" @click="noticeTo(msg.nickname,msg.mtype)" v-if="!isMe">
                                <p><Icon
																style="color: #fff; font-size: 20px;"
																type="ios-at-outline" />
                                </p>
                        ...
                        <!-- 显示隐藏图片 -->
                            <div  v-show="false">
                              <img src="../../static/images/avatar/16.gif" alt="">
                            	...
                            	<div v-if="msg.context.id === 1"
                              class="chat-msg"
                              v-html="isObject(msg.context)?msg.context.msg:msg.context"
                              @click='openimage(msg.context)'
                              @contextmenu.prevent="openMenu(msg, $event, index)"
                                >
                ...
                <!-- 用户消息 -->
								<div :id="[msg.isAiTeMeMsg?('anchor-'+msg.time):'' ]" v-if="msg.mtype !== 'user'" class="chat-item notmine " :class="[msg.flag == 2 ?'teacher':'',msg.mtype=='system'?'system':'',msg.mtype=='customer'?'customer':'',msg.mtype=='redpacket'?'redpacket':'']">
									<!-- 用户图片消息 -->
                  <div class="user-img" title="双击@他" @dblclick="noticeTo(msg.nickname,msg.mtype)">
                      ...
                      <!-- <div class="chat-info-2" v-if="(msg.mtype ==='manage')||(msg.mtype === 'system')">
                        ...
                        <div
                              :style="'left:'+left+'px'"
                               v-if="cachito === index && visible && msg.flag == 2 "
                              style="background: rgba(0, 0, 0, .8);
                                z-index: 1001;
                                cursor: pointer;
                                position: absolute; width: auto; height: 50px; display: flex; border-radius: 5px;"
                        ...
                        v-html="isObject(msg.context)?msg.context.msg:msg.context" v-if="msg.context.id == 1"
                         @click.stop='openimage(msg.context)'
                        @contextmenu.prevent="openMenu(msg, $event, index)"
                          >
												</div>
											</div>
                         ...
                         @contextmenu.prevent="openMenu(msg, $event, index)"
                          ...
                          <div
                              :style="'left:'+left+'px'"
                              v-if="cachito === index && visible && myFlag !== '1' "
                              style="background: rgba(0, 0, 0, .8);
                                z-index: 1001;
                                cursor: pointer;
                                position: absolute; width: auto; height: 50px; display: flex; border-radius: 5px;
        ...
        <div></div>
          ...
          <div style="text-align:center; font-size:16px" v-if="isPrivateRed">发给{{PrivateRedInfo.tousernickname}}</div>
			        ...
			        <FormItem label="个数：" v-if="!isPrivateRed && isShowPrivate">
    ...
    <Modal
        ...
        id="OpenImage"
        >
          <div v-html='showimage' style="text-align:center">
		...
		<Modal  class="red-info-modal" v-model="redInfoModal" footer-hide width='347px' :class="redRecordType !== 3?'red-info-modal':'red-info-modal1'">
        ...
        <!-- <div v-for="item in redRecord">
          	<div class="money bagred">


              </div>
        </div> -->

				<p class="baginformation" >已领取{{redData.redpackInfo.receivedNum}}/{{redData.redpackInfo.num}}个,共 <span class="bagred">{{redData.redpackInfo.amount|toFixed(2)}}</span> <span style="color:#333;font-size: 14px;">元</span> </p>
        ...
        <div  v-if='redRecordType ==3' class='pagetransfer' style="padding:5px" v-cloak>
        <div style="padding-top:20px">
          <img src="../../static/tctx/img/logo-logo.png" style="width:80px;padding:10px" alt="">
        </div>
        ...
        <div class='transfer'>
          ...
          <div><InputNumber v-model="alltransfervalue"  placeholder=""  :min="100" :max="myRedRecord.accountBalance" class="inputnumber" ></InputNumber>
    ...
    <Modal
      ...
      <div slot="close" style="display:none">
          <Button type="text" icon="md-close " style="display: fixed ; right:0;top:0"  size ="small" @click="cloceChat()"></Button>
			</div>
      	<div class="redlist myRedlist">
          <el-scrollbar id="outMsg" :native="false"  style="margin:2px;padding:0px" >
					<div class="message-items" style="height:388px"  >
            <!-- <el-checkbox-group
            ...
            <div v-for="(item,index) in chatRoomLists" :key="index"
            title="选择要发送的房间号"
            style="border-bottom:1px solid #F5F5F5;"
            id="checkBox"

          >
            <CheckboxGroup v-model="checkedNames" :id="item.roomId" :value="item.roomId" @on-change="checkAllGroupChange" style="padding-top:5px;">
                <Checkbox :label="item.roomId" :indeterminate ="checked" style="width:95%" >
                      <img src="../assets/images/room.png" alt=""  style="width:30px ; height:30px;  float: left">
                      <span style="color:#444 ; font-size:15px;padding-left:10px;float: left;line-height: 30px;height: 30px; ">{{item.roomName}}</span>


                </Checkbox>
            </CheckboxGroup>
						<!-- <label href="javascript:;" :for="item.roomId" style="width:90% ;display:inline-block;line-height:40px;">
            ...
            <!-- <input type="checkbox" :id="item.roomId" :value="item.roomId" v-model="checkedNames" @click="initCheckBox"  :checked="checked"  style="
            "> -->
					</div>
          <!-- <div  v-for="(item,index) in chatRoomLists" :key="index"
                title="点击进入房间"
                ...
                "
          >
              <CheckboxGroup v-model="checkedNames" @on-change="checkAllGroupChange">
                <Checkbox :label="item.roomName" :id="item.roomId">
      ...
      <div slot="footer"  >
          <div  style="height:35px">
            <Button size="large" type="default" @click.prevent.native="closeOutMessage"  style="float:left;">关闭</Button>
            <Button size="large" type="default" @click.prevent.native="changeAllChecked" v-if="checked" style="color:#EE0024 ;width:auto;">取消全选</Button>
             <Button size="large" type="default"   @click.prevent.native="changeAllChecked" v-if="!checked" style="color:#3caf36 ;width:auto;">全选</Button>
              <Button size="large" type="default"  @click.prevent.native="outMessageSend()" style="color:#EE0024;width:auto;">发送</Button>
          </div>
    </div>
    </Modal>
  ...
  import Notice from '@/components/notice.vue'
	...
	export default{
      ...
      Notice,
        ...
        '$store.state.initroom'(val){//监听用户信息事件
  				this.spinShow = this.$store.state.initroom;
  			},
        ...
        "checkedNames": function() {
          if (this.checkedNames.length == this.chatRoomLists.length) {
            this.checked = true
          } else {
            this.checked = false
          }
      ...
      this.getNotice();
        ...
        this.$root.eventHub.$on("groupToRoomList",item=>{
          this.outMessageToRoomList(item.roomList)
        });
		},
		methods:{
          outMessageToRoomList(item) {
            this.chatRoomLists = item
          },
          initCheckBox() {
          ...
          outMessageSend() {
            ...
            if(this.checked ==true) {
                console.log(this.chatRoomLists)
                for(let i = 0 ;i< this.chatRoomLists.length ; i ++) {
                  console.log(i)
                  if(this.chatRoomLists[i].roomId) {
                    let roomId = this.chatRoomLists[i].roomId.slice(7) + ','
                    this.roomIds += roomId
                    console.log(this.roomIds)

                  }else {
                    // let roomId = this.checkedNames[i].splice(0,5)
                    // console.log(roomId)
                  }
                }
                if(this.outMsgId =="" || this.roomIds == "" ) {
                        return
                }
                let outMessageToRoom = {
                  msgid:this.outMsgId,
                  roomIds:this.roomIds
                }
                console.log(outMessageToRoom)
                this.$root.sendToRoom(outMessageToRoom);
                this.roomIds =""
                this.outMsg = false;
              ...
              if(this.checkedNames.length == 0) {
                ...
                for(let i = 0 ;i< this.checkedNames.length ; i ++) {
                  if(this.checkedNames[i].roomId) {
                  }else {
                     let roomId = this.checkedNames[i].slice(7) + ','
                      this.roomIds += roomId
                      console.log(this.roomIds)
        ...
        allChecked(){
          if (!this.checked) {
            console.log(this.checked+'全选')
            this.checked = true
            this.checkedNames = this.chatRoomLists
            console.log( this.checkedNames)
            console.log( this.chatRoomLists)
          } else {
            console.log(this.checked+'全选')
      ...
      noticeTo(name,type){
       ...
       var isImg = val.msg.indexOf('<img');
         if( isImg == -1) {
           return
         }
          var bigWidth =  Math.round(document.body.offsetWidth/2)  ;
          var bigHeight  =Math.round(document.body.offsetHeight* 0.8);
          this.openImageWidth =  bigWidth +36
          this.openImageHeight =  bigHeight + 36
          var bigImage = val.msg.slice(0,val.msg.indexOf('?imageView2/2/w/340')) +'?imageView2/2/w/'+bigWidth+'/h/'+bigHeight +'">'
          // console.log(bigImage)
          this.showimage = bigImage
          this.openImage = true ;
          // if(val.width > document.body.offsetWidth &&  val.height < document.body.offsetHeight ) {
          //     if(val.width) {
          //         this.openImageWidth = document.body.offsetWidth - 36
          //         this.openImageHeight =  val.height + 36  ;
          //         this.showimage = bigImage
          //         this.openImage=true ;
          //     }else {
          //       this.showimage = bigImage
          ...
          // }else if (val.height > document.body.offsetHeight &&  val.width < document.body.offsetWidth) {
          //   if(val.width) {
          //         this.openImageWidth = val.width + 36
          //         this.openImageHeight = document.body.offsetHeight -36
          //         this.showimage = bigImage
          //         this.openImage=true ;
          //     }else {
          //       this.showimage = bigImage
      ...
      outMessage(msg) {
        ...
        const offsetLeft = this.$el.getBoundingClientRect().left; // container margin left
          this.left = e.pageX - 300 + offsetLeft ; // 15: margin right
          this.top = e.clientY;
			},
			switchUserInfo() {
			},
      ...
      //获取历史消息
		  getMessageHistory(isfrist) {
         this.isScroll = false
         if(getHistotyTime) {
           window.clearTimeout(getHistotyTime);
         }
        var getHistotyTime = window.setTimeout(()=>{
            this.isScroll = true
            },10000);
        ...
        if(isfrist) {
           this.getHistotyTime = '';
				...
				if(this.messageList.length !== 0) {
					this.getHistotyTime = this.messageList[0].time?this.messageList[0].time:this.messageList[0].time+''
      }
      console.log(this.getHistotyTime)
				RedEnvelope.getHistory(this.getHistotyTime).then(
					res => {
      ...
      privateRedOpen(privateId,isShowPrivate){
				if(!Constants.isLogin){
					this.$root.eventHub.$emit("showLoginModelEvent");
				}else{
