		    ...
		    <Tabs v-model="tabName" >
				<TabPane label="用户资料" name="name1">
					<div class="info-body">
						<ul>
              <li>
								<p>余额：</p><p style="color: #666">{{balance}}</p><p  style="margin-left:50px;border:1px solid #e4393c ;border-radius:3px ;padding:0 5px 0 5px ;color:#e4393c" @click="transfer" v-if="balance > '0' && userInfo.userFlag !==2">转账到平台</p>
							</li>
							<li>
							...
							<li>
								<p>用户组：</p><p style="color: #666">{{userInfo.userFlag === 2?'管理员用户组':'普通用户注册组'}}</p>
							</li>
						</ul>
					</div>
				</TabPane>
	...
	export default{
      ...
      transfer() {
        ...
        if ( !this.binded) {
           ...
           this.$parent.MyRedClick(3);
            this.isTransfer = false;
        }

      },
      bind() {
          if (this.userInfo.platformAccount !== ""
          && typeof this.userInfo.platformAccount !== "undefined" &&  this.userInfo.platformAccount !== null) {
              changeInfo.subInfo({
