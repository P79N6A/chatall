<template>
	<div>
       ...
       :draggable="fullscreen?false:true"
        v-model="privateChatModal"
         :fullscreen="fullscreen"
          ...
          <!-- <span class="unicks">{{myChatRooms.roomName}}</span> -->
      ...
      <div  class="privateChatUser">
          ...
          <ul  >
            <li v-for="item in userDegree" @click.prevent="openPrivateChat(item,item.userid)" :key="(item.userid) "
      ...
      <div style="" class="message-box">
        <div class="message-header">
            {{noticeName}}
            <span style="color:#3cAF36"  v-if="roomFlag === '1'">
              {{roomName}}
            </span>
            ...
            >
							...
							<div class="chat-info">
								...
								<div class="chat-info-2">
									<div class="chat-msg" v-if ="item.message.id == '1'" v-html="item.message.msg"  @click='privateOpenImage(msg.context)' >
                  ...
                  <div class="chat-msgs" v-if ="item.message.id == null&& item.message.redpacketid" @click="openMyPrivateRed(item.fromid,item.message.remarks,item.message.redpacketid)" >
                    ...
                    <div class="message-wrap" v-if="item.type=='red' && msg.redpackettype == '0' ">
    ...
    computed:{
      ...
      this.$root.eventHub.$on("showChatListChatEvent",fromid =>{
        //私聊点开
            ...
            console.log(nowItem)
            this.roomName = nowItem.roomname
            this.roomFlag = nowItem.flag
      ...
      this.$root.eventHub.$on("PrivateChatMyRedReceiveEvent",item=>{
		...
		methods:{
      privateOpenImage(imgItem) {
         this.$parent.openimage(imgItem)
      },
      uploadyanzheng(file) {
      ...
      privateRedOpen(){
        this.$parent.privateRedOpen(this.$store.state.nowid,false)
      },
      settings() {
      ...
      FormattedMessages(item){
              if(item.message.id == '1') { //id为1其实是图片消息
                ...
                item.message.msg = '<img src="http://' + cut + '?imageView2/2/w/340">'
              }
              else {
      ...
      openPrivateChat(item,toId) {
        ...
        if(!this.$store.state.noticename){
            ...
            this.$store.commit('NOTICE_ROOMNAME',item.roomName);
          ...
          this.$store.state.roomname = item.roomName;
          this.noticeName = item.nickname
          if(item.roomName) {
               this.roomName = item.roomName
          }
