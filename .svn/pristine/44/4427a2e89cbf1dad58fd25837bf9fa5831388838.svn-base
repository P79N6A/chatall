		...
		<div class="header">
			...
			<div class="header-right">
				...
				<template>
					...
					<template v-else>
						<!-- <a href="javascript:void(0)" class="userlogin">
              <img :src="imgUrl+'images/201709080109531642.png'" border="0" style="width:20px; height:20px;vertical-align: middle;">
              游客</a> -->
			...
			<div class="modal-content rel">
		        ...
		        <div class="modal-c mt40">
		        	<Form :model="loginForm">
		        		...
		        		<FormItem class="mb20">
		        			<Button class="btn" style="width:100%;" type="primary" :loading="loginLoading" @click="login" >立即登录</Button>
		        		</FormItem>
		        	</Form>
		        	<div>
  ...
  import Cookie from '@/utils/auth'
	...
	export default{
        ...
        this.changeCode() ;
			...
			submitNewPwd() {
				...
				if(this.changeUserPwd.newPassword.trim().length <6) {
					this.$Message.error('新密码密码长度不能小于位！');
					return false
				}
      ...
      //注册
          ...
          if(!patt.test(this.registForm.username)) {
              this.showLoginErrorTxt = '会员账号名称由字母数字组成';
              return
          }
        ...
        let  md5_loginPassword =md5.makeMd5(this.loginForm.password)
          ...
          Cookie.setToken(data.token);
           Cookie.SetExpireTime(data.expire);
            ...
            that.$root.eventHub.$emit("onLiveChatEvent",that.userInfo);
             this.$router.push({name: 'index', params: {uid: data.user.userId}})
				}).catch(error =>{
					that.loginLoading = false;
      ...
      //登出
          ...
          this.$store.commit("UPDATE_LOGINSTATUS",true);
