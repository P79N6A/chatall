...
export default {
    ...
    UPDATE_LOGINSTATUS(state, newStatus) {
      ...
      ROOM_ID(state,newroomid) {
        state.newroomid = newroomid
      },
      GET_ROOM_INFO(state,getroominfo) {
        state.getroominfo = getroominfo
      },
      SET_INIT_ROOM(state,initroom) {
        state.initroom = initroom
      },
      NOTICE_ROOMNAME(state,newroomName) {
